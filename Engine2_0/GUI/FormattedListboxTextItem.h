...
public:
	...
	*/
	...
	// overriden functions.
	Size<float> getPixelSize(void) const;
	void draw(GeometryBuffer& buffer, const Rect<float>& targetRect,
			  float alpha, const Rect<float>* clipper) const;
...
protected:
	...
	mutable Size<float> d_formattingAreaSize;
};
